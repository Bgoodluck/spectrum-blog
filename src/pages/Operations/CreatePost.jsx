...
function CreatePost() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(summaryApi.buildPost.url, {
        method: summaryApi.buildPost.method,
        headers: {
      ...
      if (!data.success) {
        // If post creation fails, cleanup the uploaded media
        if (mediaData.url) {
          try {
            // Extract path from the URL or store it during upload
            const pathParts = new URL(mediaData.url).pathname.split('/');
            const path = pathParts.slice(pathParts.indexOf('jobs-board-public') + 1).join('/');
            await deleteFile(path, 'jobs-board-public');
          } catch (deleteError) {
      ...
      <div className="relative inline-block mb-4">
        {mediaType === 'image' ? (
          <img
            src={mediaPreview}
            alt="Preview"
            className="max-h-40 w-full object-cover rounded dark:opacity-80"
          />
        ) : (
          <video
            src={mediaPreview}
        ...
        <button
          type="button"
          onClick={clearMedia}
          className="absolute top-2 right-2 p-1 bg-red-500 text-white rounded-full hover:bg-red-600 dark:bg-red-700 dark:hover:bg-red-600"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
    ...
    <Tooltip content="Insert Video in Text">
      ...
      >
        ...
        <div className="lg:col-span-1">
          <Card className="sticky top-4 dark:bg-gray-800">
            <h2 className="text-xl font-semibold mb-4 dark:text-white">Preview</h2>
            {isPreviewMode ? (
              <div className="prose dark:prose-invert max-w-none">
                <h1 className="dark:text-white">{title || 'Post Title'}</h1>
                {mediaPreview && (
                  <div className="mb-4">
                    {mediaType === 'image' ? (
                      <img
                        src={mediaPreview}
                        alt="Preview"
                        className="rounded max-h-60 w-full object-cover dark:opacity-80"
                      />
                    ) : (
                      <video
                        src={mediaPreview}
